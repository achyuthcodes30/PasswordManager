...
def connect():
    try:
        connection = psycopg2.connect(user='postgres', password='postgresql', host='127.0.0.1', database='password_manager')
        return connection
    except (Exception, psycopg2.Error) as error:
        print(error)
...
def store_masterpassdb(provided_password):
    app_name="BookFace"
    try:
        connection = connect()
        cursor = connection.cursor()
        postgres_insert_query = """ INSERT INTO masterpass(masterpassword,app_name) VALUES (%s,%s) ON CONFLICT (app_name) DO UPDATE SET masterpassword='""" + provided_password + "'"
        record_to_insert = (provided_password,app_name)
        cursor.execute(postgres_insert_query, record_to_insert)
        connection.commit()
    except(Exception, psycopg2.Error) as error:
        print(error)

def get_masterpassdb():
    try:
        connection = connect()
        cursor = connection.cursor()
        postgres_select_query = """ SELECT masterpassword FROM masterpass WHERE app_name = '""" + app_name + "'"
        cursor.execute(postgres_select_query, app_name)
        connection.commit()
        result = cursor.fetchone()
        return(result[0])
